...
// 计算两点之间的长度
double CFractalDragon::GetLength(CPoint &pt1, CPoint &pt2) {
    return sqrt((pt1.x - pt2.x) * (pt1.x - pt2.x) + (pt1.y - pt2.y) * (pt1.y - pt2.y));
}

// 计算两点之间的中点
CPoint CFractalDragon::GetCenter(CPoint &pt1, CPoint &pt2) {
    return CPoint((pt1.x + pt2.x) / 2, (pt1.y + pt2.y) / 2);
}

// 判断点C 在向量AB的左侧还是右侧
bool CFractalDragon::IsLeft(CPoint &ptA, CPoint &ptB, CPoint &ptC) {
    bool isLeft = false;
...
CPoint CFractalDragon::GetVerticalCenter(CPoint &start, CPoint &end, bool needLeft) {
...
VecPt CFractalDragon::GetVertexs(int32_t step, CPoint start, CPoint end, bool needLeft) {
    if (step > 0) {
        ...
        step--;
        // 获得垂中
        CPoint verCenter = GetVerticalCenter(start, end, needLeft);
        // 根据 start  verCenter  end 劈成两半
        // 左边
        VecPt ptVecLeft = GetVertexs(step, start, verCenter, true);
        // 右边
        VecPt ptVecRight = GetVertexs(step, verCenter, end, false);
        // 左边和右边的拼接起来
